...
PNG brighten(PNG original, int amount)
{
    ...
    for (int yi = 0; yi < original.height(); yi++)
    {
		for (int xi = 0; xi < original.width(); xi++)
        {
            ...
            {
                original(xi, yi)->red += amount; // Increase red  by amount
            } else {
                original(xi, yi)->red = 255; // Overflow
            }
            if (original(xi, yi)->green + amount <= 255)
            {
                original(xi, yi)->green += amount; // Increase green  by amount
            } else {
                original(xi, yi)->green = 255; // Overflow
            }
            if (original(xi, yi)->blue + amount <= 255)
            {
                original(xi, yi)->blue += amount; // Increase blue  by amount
            } else {
                original(xi, yi)->blue = 255; // Overflow
            }
	    }
    }
    return original;
...
PNG blendImages(PNG firstImage, PNG secondImage)
{
    for (int yi = 0; yi < min(firstImage.height(), secondImage.height()); yi++)
    {
		for (int xi = 0; xi < min(firstImage.width(), secondImage.width()); xi++){
            firstImage(xi, yi)->red = (firstImage(xi, yi)->red + secondImage(xi, yi)->red)/2;       // Take the average of red and update it
            firstImage(xi, yi)->green = (firstImage(xi, yi)->green + secondImage(xi, yi)->green)/2; // Take the average of green and update it
			firstImage(xi, yi)->blue = (firstImage(xi, yi)->blue + secondImage(xi, yi)->blue)/2;    // Take the average of blue and update it
	    }
    }
    return firstImage;
}
